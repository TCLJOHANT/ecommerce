        ...
        <div class="flex flex-col md:flex-row gap-4">
                        ...
                        <tbody>
                            @forelse ($cart_items as $item)
                                <tr wire:key="{{$item['product_id']}}">
                                    ...
                                    <td class="py-4">{{Number::currency($item['unit_amount'],'COL')}}</td>
                                    <td class="py-4">
                                        <div class="flex items-center">
                                            <button wire:click="decreseQty({{$item['product_id']}})" class="border rounded-md py-2 px-4 mr-2">-</button>
                                            <span class="text-center w-8">{{$item['quantity']}}</span>
                                            <button wire:click="increaseQty({{$item['product_id']}})" class="border rounded-md py-2 px-4 ml-2">+</button>
                                        </div>
                                    </td>
                                    <td class="py-4">{{Number::currency($item['total_amount'],'COL')}}</td>
                                    <td>
                                        <button wire:click="removeItem({{$item['product_id']}})" class="bg-slate-300 border-2 border-slate-400 rounded-lg px-3 py-1 hover:bg-red-500 hover:text-white hover:border-red-700">
                                            <span wire:loading.remove wire:target="removeItem({{$item['product_id']}})">Remover</span><span  wire:loading wire:target="removeItem({{$item['product_id']}})" class="hidden">Removiendo...</span>
                                        </button>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4 text-4xl font-semibold text-slate-500">No hay Items en el carrito</td>
                                </tr>
                            @endforelse
                            <!-- More product rows -->
                        </tbody>
            ...
            <div class="md:w-1/4">
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="flex justify-between mb-2">
                        <span>Taxes</span>
                        <span>{{Number::currency(0,'COL')}}</span>
                    </div>
                    <div class="flex justify-between mb-2">
                        <span>Shipping</span>
                        <span>{{Number::currency(0,'COL')}}</span>
